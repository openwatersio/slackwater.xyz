...
- **`skyPoint`'s 360° fallback tears the sky at the pole.** With no `HorizonSpan` the window is the whole horizon centred on the meridian, which puts its seam due north in the northern hemisphere — exactly where the celestial pole is — and circumpolar constellations are drawn split across both edges of the frame. Deception Pass cannot reach it, because the sun's span is always defined across this page's window; a polar station, or any station-day with no rise and set pair, reaches it immediately.

  The fix costs one branch on which event is missing, and nothing is traded for it. `starOpacity` is zero unless the sun is more than 6° down, so under a polar day no star is ever drawn and the seam cannot hurt anything; under a polar night stars are drawn at full strength and there is no body sweeping to disturb. So: no set (polar day) keeps today's centring, and no rise (polar night) centres the window on the pole instead, putting the seam as far from the circumpolar stars as it goes. Decided once per day from which event is missing, so the frame never jumps. Measured at Tromsø: 21 June, zero rise/set events, altitude +3.3° to +43.8°, star opacity 0.00 throughout; 21 December, zero events, −43.5° to −2.5°, star opacity reaching 0.70.

  A sun disc *is* drawn during part of the polar night — `SUN_FLOOR_DEG` is −8/3 ≈ −2.67°, and Tromsø's sun clears it for 69 minutes on the solstice and over three hours at the ends of the season, when it peaks nearer the floor. That does not weaken the rule, because the disc and the star field can never be drawn at the same instant: one needs the sun above −2.67° and the other needs it below −6°, and the two conditions are disjoint by construction. The argument holds for the whole polar-night season at every latitude, and does not depend on the window being short.

  `SUN_FLOOR_DEG` is not an invention of this port either — it is `sunDiscRadius` 8 over `skyAltitudeScale` 3, the altitude at which the disc stops poking above the horizon line. `Theme.swift` has no explicit floor because it relies on its clip instead.

  This lives in `slackwater-ios`, which is the source of truth for the projection, so it is an issue to raise there rather than a patch to make here.
