...
- **`skyPoint`'s 360° fallback tears the sky at the pole.** With no `HorizonSpan` the window is the whole horizon centred on the meridian, which puts its seam due north in the northern hemisphere — exactly where the celestial pole is — and circumpolar constellations are drawn split across both edges of the frame. Deception Pass cannot reach it, because the sun's span is always defined across this page's window; a polar station, or any station-day with no rise and set pair, reaches it immediately.
  ...
  One caveat on that second case: `SUN_FLOOR_DEG` is −2.67°, and Tromsø's 21 December sun clears it for about 70 minutes a day, so a static sun is drawn for that window. Star opacity is 0 throughout it, so nothing tears — but "no body is drawn during polar night" is very nearly true rather than exactly true. This lives in `slackwater-ios`, which is the source of truth for the projection, so it is an issue to raise there rather than a patch to make here.
