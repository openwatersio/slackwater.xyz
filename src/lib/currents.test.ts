...
import { HERO_STATION, findEvents, nextEvent, predictSeries } from './currents'
...
describe('hero station currents', () => {
  ...
  it('predicts a curve that actually moves, without a staircase', () => {
    const s = predictSeries(START, 24)
    ...
    const distinct = new Set(s.map((x) => x.knots)).size
    expect(distinct).toBeGreaterThan(s.length * 0.95)
    const speeds = s.map((x) => Math.abs(x.knots))
  ...
  it('finds slack at the zero crossings and extremes between them', () => {
    const events = findEvents(START, 24)
    ...
    for (const e of events.filter((x) => x.kind === 'slack')) {
      expect(Math.abs(e.knots)).toBeLessThan(0.001)
    }
    // Flood is signed positive, ebb negative — one axis, not two colours.
    for (const e of events.filter((x) => x.kind === 'flood')) expect(e.knots).toBeGreaterThan(0)
    for (const e of events.filter((x) => x.kind === 'ebb')) expect(e.knots).toBeLessThan(0)
